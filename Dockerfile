...
RUN set -x && \
    ...
    rm -f /etc/apt/sources.list.d/openjdk-r-ubuntu-ppa-disco.list && \
    echo -e "deb http://ppa.launchpad.net/openjdk-r/ppa/ubuntu devel main \n# deb-src http://ppa.launchpad.net/openjdk-r/ppa/ubuntu devel main" > openjdk-r-ubuntu-ppa-disco.list && \
    apt-get update && \
    apt-get install -y --no-install-recommends --force-yes openjdk-8-jdk && \
    apt-get clean && \
    apt-get autoremove && \
...
RUN /bin/bash -c 'set -x && \
    ...
    wget -qO - http://www.apache.org/dist/sqoop/1.4.7/sqoop-1.4.7.bin__hadoop-2.6.0.tar.gz | tar -xz -C /opt && \
    cd /opt && ls -la && \
    cd /opt && ln -s ./sqoop-1.4.7.bin__hadoop-2.6.0.tar.gz sqoop && ls -la && \
    #mkdir /opt/sqoop/lib && \
